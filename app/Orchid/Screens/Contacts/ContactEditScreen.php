...
class ContactEditScreen extends Screen
{
    ...
    {
        $contact->load(['leads.comments.user', 'createdBy']);
        $this->contact = $contact;

        $comments = $contact->exists
            ? $contact->leadComments()
                ->with(['lead', 'user'])
                ->latest('lead_comments.created_at')
                ->get()
            : collect();
    ...
    public function layout(): iterable
    {
        return [
            ...
            Layout::tabs([
                ...
                'Leads' => Layout::view('admin.contacts.leads'),
                'Comments' => Layout::view('admin.contacts.comments'),
                'Traffic summary' => Layout::view('admin.contacts.traffic-summary'),
            ]),
        ];
    }
